...
function pickBestPriceRow(
  rows: any[],
  nomenclatureId: number,
  _priceTypeId?: number | null,
): any | null {
  if (!rows.length) return null;

  const exactByNomenclature = rows.filter((row) => {
    const rowNomenclatureId = getRowNomenclatureId(row);
    return rowNomenclatureId === nomenclatureId;
  });

  // Критично: если нет точного совпадения по номенклатуре,
  // нельзя брать цену от чужого товара.
  if (exactByNomenclature.length === 0) {
    return null;
  }

  const nonZero = exactByNomenclature.find((row) => getPriceValue(row) > 0);
  if (nonZero) return nonZero;

  return exactByNomenclature[0] ?? null;
}

function extractPriceFromEmbeddedPrices(
  item: any,
  nomenclatureId: number,
...
async function findPriceForNomenclature(
  ...
  try {
    ...
    console.error("PRICE LOOKUP ERROR:", {
      nomenclatureId,
      priceTypeId,
      error,
    });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const filtered = filterNomenclature(array, term);
